...
grid_step = 1.0 if is_m else 100.0 # 정확한 1m 그리드 간격 강제 지정
...
if dist_speakers <= 0: 
    ...
    # Top View
    at.set_aspect('equal', adjustable='box') # 1:1 비율 강제 적용
    at.xaxis.set_major_locator(ticker.MultipleLocator(grid_step)) # 1m 단위 그리드 픽스
    ...
    # Side View
    aside.set_aspect('equal', adjustable='box') # 1:1 비율 강제 적용
...
if dist_speakers > 0:
    ...
    # [Top View: 룸 트리트먼트]
    trt_top.set_aspect('equal', adjustable='box') # 1:1 비율 강제
    trt_top.xaxis.set_major_locator(ticker.MultipleLocator(grid_step)) # 정확한 1m 그리드 강제
    ...
    # LP 좌표 텍스트 추가 (Treatment Top View)
    ...
    # [Side View: 룸 트리트먼트]
    trt_side.set_aspect('equal', adjustable='box') # 1:1 비율 강제
    trt_side.xaxis.set_major_locator(ticker.MultipleLocator(grid_step)) # 정확한 1m 그리드 강제
    ...
    # LP 좌표 텍스트 추가 (Treatment Side View)
